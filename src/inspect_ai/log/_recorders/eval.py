...
class EvalRecorder(FileRecorder):
    ...
    async def _read_log_sample_impl(
        ...
        # outside the handler so a malformed archive reports itself rather than
        # chaining onto the missing monolith member
        from .chunked.read import read_chunked_sample

        return await read_chunked_sample(
            reader, names, id, epoch, location, exclude_fields
        )
